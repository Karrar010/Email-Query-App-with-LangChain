"""
Simple Email Query App - No LangChain Dependencies
"""
...
from simple_email_search_no_langchain import SimpleEmailStore, SimpleEmailQA
...
def main():
    ...
    st.set_page_config(
        page_title="Email Query App - Simple Version",
        page_icon="📧",
        layout="wide",
    ...
    # Main title
    st.title("📧 Email Query App - Simple Version")
    st.markdown("*Lightweight version without LangChain dependencies*")
    st.markdown("---")
    
    # Check if Groq API key is configured
    if not GROQ_API_KEY:
        st.error("⚠️ Groq API key not configured. Please set GROQ_API_KEY in your .env file.")
        st.stop()
    ...
    with col1:
        ...
        if st.session_state.emails:
            ...
            if summary['senders']:
                ...
                st.dataframe(sender_df, width='stretch')
                        ...
                        if result['sources']:
                            ...
                            for i, source in enumerate(result['sources'], 1):
                                with st.expander(f"Email {i}: {source['subject']}"):
                                    ...
                                    st.write(f"**Date:** {source['received']}")
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #666;'>
            <p>Simple Email Query App | Built with Streamlit, Microsoft Graph API, and Groq</p>
        </div>
        """,
